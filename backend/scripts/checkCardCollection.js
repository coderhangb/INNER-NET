...
async function main() {
  ...
  if (
    ...
    throw new Error("Provide two different seeded user IDs");
  ...
  async function get(path, id) {
    const response = await fetch("http://localhost:3000/api/cards" + path, {
      headers: id ? { Cookie: cookies[id] } : {},
  ...
  assert.ok(
    A.data.items.length >= 12 && B.data.items.length >= 12,
    "Seed both users first",
  );
  ...
  assert.ok(
    filtered.data.items.every(
      (x) => x.metadataSnapshot.rarity === "common" && x.status === "available",
    ),
  );
  ...
  for (let page = 0; page < 1000; page++) {
    const result = await get(
      "/me?limit=5" + (cursor ? "&cursor=" + cursor : ""),
      a,
    );
    ...
    assert.equal(
      result.data.total,
      total,
      "Collection changed during test; use a stable fixture",
    );
    for (const item of result.data.items) {
      assert.ok(!seen.has(item._id), "Duplicate pagination ID");
      seen.add(item._id);
    }
  ...
  assert.ok(templates.data.items.length >= 10);
  const groups = new Map();
  for (const c of A.data.items)
    groups.set(c.templateId, (groups.get(c.templateId) || 0) + 1);
