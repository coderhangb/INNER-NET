...
const fields =
  "_id templateId templateVersion metadataSnapshot status createdAt";
async function listMine(req, res, next) {
  try {
    const { limit, cursor } = paging(req.query, [
      "cursor",
      "limit",
      "rarity",
      "status",
...
async function getMine(req, res, next) {
  try {
    ...
    const item = await CardInstance.findOne({
      _id: req.params.id,
...
async function listTemplates(req, res, next) {
  try {
    ...
    const rows = await CardTemplate.find(filter)
      .select("_id slug version name rarity description symbol imageUri")
      .sort({ _id: -1 })
      .limit(limit + 1)
      .lean();
    const items = rows.slice(0, limit);
    return res.json({
      items,
      nextCursor: rows.length > limit ? String(items.at(-1)._id) : null,
    });
  } catch (error) {
    if (error.message === "INVALID_QUERY")
