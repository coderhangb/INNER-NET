...
function createActivityService({
  ...
  async function student(userId, session) {
    ...
    if (u.role !== "student")
      throw fault(
        403,
        "STUDENT_ONLY",
        "Chỉ tài khoản học sinh được nhận card.",
      );
  }
      ...
      await Progress.updateOne(
        { _id: userId },
        {
          $setOnInsert: {
            ...
            dayKey: businessDay(clock.now()).key,
          },
        },
        { upsert: true },
      );
  ...
  function checkConfig(p) {
    if (p && p.configVersion !== rules.configVersion)
      throw fault(
        409,
        "PROFILE_MISMATCH",
        "Tiến độ thuộc cấu hình khác. Khôi phục profile cũ hoặc dùng tài khoản thử mới.",
      );
  }
  async function response(userId, extra = {}) {
    ...
    const recentGrants = await Grant.find({ userId })
      .sort({ milestone: -1 })
      .limit(20)
      .select(
        "_id milestone cardInstanceId name symbol rarity configVersion createdAt",
      )
      .lean();
    return {
      profile: rules.profile,
      configVersion: rules.configVersion,
      intervalSeconds: rules.intervalMs / 1000,
      heartbeatSeconds: rules.heartbeatMs / 1000,
      idleSeconds: rules.idleMs / 1000,
      leaseSeconds: rules.leaseMs / 1000,
      remainderSeconds: (p?.remainderMs || 0) / 1000,
      dailyCount,
      dailyCap: rules.dailyCap,
      nextMilestone: p?.nextMilestone || 1,
      timezone: rules.timezone,
      pauseReason:
        dailyCount >= rules.dailyCap
          ? "daily_cap"
          : p?.pauseReason === "daily_cap"
            ? null
            : p?.pauseReason || null,
      recentGrants,
      ...extra,
    ...
    try {
      await session.withTransaction(
        async () => {
          ...
          if (!p)
            throw fault(409, "SESSION_EXPIRED", "Hãy mở phiên hoạt động mới.");
          checkConfig(p);
  ...
  async function open(userId) {
    ...
    return transact(userId, async (p, session, now) => {
      if (p.activeSessionId && +p.leaseUntil > now)
        return { sessionId: null, leaseBusy: true };
      rollover(p, now);
      p.activeSessionId = sessionId;
      p.lastHeartbeatAt = null;
      p.lastSequence = 0;
      p.leaseUntil = new Date(now + rules.leaseMs);
      p.pauseReason = null;
      p.version++;
  ...
  async function heartbeat(userId, body) {
    ...
    let draw = null;
    return transact(userId, async (p, session, now) => {
      if (p.activeSessionId !== body.sessionId)
        throw fault(409, "SESSION_EXPIRED", "Phiên hoạt động đã thay đổi.");
      if (body.sequence <= p.lastSequence) return { duplicate: true };
      if (+p.leaseUntil <= now)
        throw fault(409, "SESSION_EXPIRED", "Phiên hoạt động đã hết hạn.");
      rollover(p, now);
      p.lastSequence = body.sequence;
      if (!body.visible || !body.active) {
        p.lastHeartbeatAt = null;
        p.leaseUntil = new Date(now);
        p.activeSessionId = null;
        p.pauseReason = "inactive";
        p.version++;
        await p.save({ session });
        return {};
      }
      credit(p, now, rules);
      p.lastHeartbeatAt = new Date(now);
      p.leaseUntil = new Date(now + rules.leaseMs);
      p.pauseReason = null;
      if (p.dailyCount >= rules.dailyCap) {
        ...
        const pool = await Template.find({ active: true })
          .sort({ _id: 1 })
          .session(session)
          .lean();
        const valid =
          validWeights(rules.weights) &&
          rules.weights.every(([r]) => pool.some((t) => t.rarity === r));
        if (!valid) {
          ...
          if (!draw) {
            ...
            draw = {
              template: group[rng.randomInt(group.length)],
              grantId: new mongoose.Types.ObjectId(),
          ...
          const t = pool.find(
            (t) => String(t._id) === String(draw.template._id),
          );
          if (!t) p.pauseReason = "reward_unavailable";
              ...
              [
                {
                  _id: draw.eventId,
                  cardId: draw.cardId,
                  operationId: `reward:${draw.grantId}`,
            ...
            p.remainderMs -= rules.intervalMs;
            p.nextMilestone++;
            p.dailyCount++;
            if (p.dailyCount >= rules.dailyCap) {
              p.remainderMs = 0;
