...
function businessDay(now) {
  return { key: new Date(now + OFFSET).toISOString().slice(0, 10), start: Math.floor((now + OFFSET) / DAY) * DAY - OFFSET };
}
function rollover(p, now) {
  const day = businessDay(now);
  if (p.dayKey !== day.key) { p.dayKey = day.key; p.dailyCount = 0; }
  return day;
}
function credit(p, now, rules) {
  const day = rollover(p, now);
  if (p.dailyCount >= rules.dailyCap) { p.remainderMs = 0; return 0; }
...
function validWeights(weights) {
  return weights.length === 5 && new Set(weights.map(x => x[0])).size === 5 &&
    weights.every(x => Number.isInteger(x[1]) && x[1] > 0) && weights.reduce((n,x) => n+x[1],0) === 100;
}
function pickRarity(draw, weights) {
  if (!validWeights(weights) || !Number.isInteger(draw) || draw < 0 || draw >= 100) throw new Error('Invalid reward draw/config');
  let end = 0;
  for (const [rarity, weight] of weights) { end += weight; if (draw < end) return rarity; }
}
